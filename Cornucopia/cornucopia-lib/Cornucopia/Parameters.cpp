...
void Parameters::_initializePresets()
{
	...
	//lines and arcs
    Parameters newlinesarcs = defaultParams;
    newlinesarcs._name = "Lines And Arcs";
    newlinesarcs.set(LINE_COST, 15.);
    newlinesarcs.set(ARC_COST, 20.);
    newlinesarcs.set(CLOTHOID_COST, infinity);
    newlinesarcs.set(G0_COST, 0.);
    newlinesarcs.set(G1_COST, infinity);
	newlinesarcs.set(G2_COST, infinity);
    newlinesarcs.set(SHORTNESS_COST, 0);
    _presets[NEW_LINES_AND_ARCS] = newlinesarcs;
}
