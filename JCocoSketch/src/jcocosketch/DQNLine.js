...
(function(global) {
  ...
  Graph.prototype = {
    ...
    tanh: function(m) {
      ...
      var n = m.w.length;
      for(var i=0;i<n;i++) { 
        out.w[i] = java.lang.Math.tanh(m.w[i]);
      }
...
function actDQNLine(x, y) {
  var action = agent.act([x,y]);
  return action
}

function learnDQNLine(rewardValue) {
...
function start() { // this is a test function
  ...
  var action = agent.act([x,y]); // s is an array of length 8
