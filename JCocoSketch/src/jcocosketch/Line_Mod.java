...
public class Line_Mod {
	...
	public Line Segment(Line line, int samples, boolean variant) {
		...
		for (int i = 0; i < line.allPoints.size(); i+=samples) {
			...
			input.add((double)line.allPoints.get(i).x);
			input.add((double)line.allPoints.get(i).y);
			inputList.add(input);
			if (!variant) {
	...
	public Line generateBYNEATLEARNING(Line line, int samples) {
		...
		for (int i =0; i < line.allPoints.size(); i+=samples){
			...
			double[] data = {(double)line.allPoints.get(i).x, (double)line.allPoints.get(i).y};
			pf.coeffs[3] = 200;
	...
	public Line generateBYCTMExploration(Line line) {
		...
		for (int i =0; i <allY.length; ++i) {
			allY[i] = (int)line.allPoints.get(i).y;
		...
		for (int i =0; i <newY.length; ++i) {
			newLine.allPoints.add(new Point(line.allPoints.get(i).x, newY[i], newLine.lineID));
		}
